...
async function performAction(rawArgs) {
    ...
    if (firstArg === 'run') {
        ...
        await execute(
            `HARDHAT_DEPLOY_LOG=true HARDHAT_NETWORK=${fixedArgs[0]} ts-node --files ${filepath} ${extra.join(' ')}`
        )
        ...
        await execute(`hardhat  export --export-all contractsInfo.json`)
        ...
        await execute(
            `${options.deploy ? 'HARDHAT_DEPLOY_FIXTURE=true' : ''} HARDHAT_DEPLOY_LOG=true HARDHAT_FORK=${
                fixedArgs[0]
        ...
        await execute(
            `HARDHAT_FORK=${fixedArgs[0]} ${options.blockNumber ? `HARDHAT_FORK_NUMBER=${options.blockNumber}` : ''} ${
                options['no-impersonation'] ? `HARDHAT_DEPLOY_NO_IMPERSONATION=true` : ''
        ...
        await execute(`hardhat node  --fork ${process.env[`NODE_URI_${fixedArgs[0].toUpperCase()}`]}`)
        ...
        await execute(
            `HARDHAT_FORK=${fixedArgs[0]} ${options.blockNumber ? `HARDHAT_FORK_NUMBER=${options.blockNumber}` : ''} ${
                options['no-impersonation'] ? `HARDHAT_DEPLOY_NO_IMPERSONATION=true` : ''
            } hardhat node --hostname 0.0.0.0 --watch  ${extra.join(' ')}`
        )
    }
}
